...
double Kinetic() { //Write Function here!  
    ...
    #pragma omp parallel for private(v2) reduction(+:kin)
    for (int i=0; i<N; i++) {
...
double computeAccelerationsPotential() {
    int i, j, k;
    double f, rSqd, val, vall, Pot = 0.;
    double vals[3], rij[3]; // position of i relative to j
    ...
    #pragma omp parallel for schedule(dynamic, 40) private(j, f, rSqd, val, vall, vals, rij) reduction(+:Pot, a)
    for (i = 0; i < N-1; i++) {   // loop over all distinct pairs i,j
        for (j = i+1; j < N; j++) {
            ...
            rij[0] = r[i][0] - r[j][0]; rij[1] = r[i][1] - r[j][1]; rij[2] = r[i][2] - r[j][2];

            //  sum of squares of the components
            rSqd = rij[0]*rij[0] + rij[1]*rij[1] + rij[2]*rij[2];

            //  local variables to save costly calculations
            val = rSqd * rSqd * rSqd;    
            vall = 1/(val * val * rSqd);

            //  Increasing the potential value
            Pot += 4 * (rSqd - (val*rSqd)) * vall;
            ...
            //  Updating the acceleration matrix
            a[i][0] += vals[0]; a[i][1] += vals[1]; a[i][2] += vals[2];
            a[j][0] -= vals[0]; a[j][1] -= vals[1]; a[j][2] -= vals[2];
        }
    }
